  ...
  return (
    <section className={`w-full ${TONE_CLASS[tone]}`} style={!o?.tone ? { background: "var(--dp-dark, #111)" } : undefined}>
      <div
        ...
        style={{ paddingTop: `${4.6 * pad}rem`, paddingBottom: `${1.6 * pad}rem` }}
      >
        <h1
          className="mx-auto max-w-[92%] font-extrabold leading-[1.28] tracking-[-0.025em]"
          style={{ fontSize: `${46 * (o?.headlineScale ?? 1)}px` }}
        >
          {copy.headline}
        </h1>
        {copy.subheadline && (
          <p className={`mx-auto mt-5 max-w-[560px] text-[17px] leading-[1.7] ${dark ? "text-white/55" : "text-ink-mute"}`}>
            {copy.subheadline}
          </p>
        )}
        {copy.bullets && copy.bullets.length > 0 && (
          <div className="mt-6 flex flex-wrap justify-center gap-x-5 gap-y-1.5 text-[14px]">
      ...
      {copy.cta && (
        <div className="mx-auto w-full max-w-detail px-10" style={{ paddingBottom: `${3.4 * pad}rem` }}>
          <span
            className="flex w-full items-center justify-center rounded-2xl px-6 py-5 text-[17px] font-bold text-white"
