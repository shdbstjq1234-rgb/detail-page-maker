    ...
    <SectionShell tone="light">
      ...
      {rows.length > 0 && (
        <dl className="mt-8 overflow-hidden rounded-2xl border border-line">
          {rows.map((r, i) => (
            <div
              key={i}
              className={`flex gap-5 px-5 py-4 text-[15px] ${i % 2 ? "bg-white" : "bg-[color:var(--dp-bg-alt,#f7f6f3)]"}`}
            >
      ...
      {notices.length > 0 && (
        <div className="mt-6 rounded-2xl bg-ink px-5 py-6 text-white">
          <p className="text-[13px] font-bold uppercase tracking-[0.16em] text-white/50">구매 전 확인해 주세요</p>
